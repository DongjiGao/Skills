...
def test_mmau_pro_audio_prefix(monkeypatch):
    ...
    entry = {
        ...
        "audio_path": ["data/audio/example.wav", "/already/absolute.wav"],
    }

    formatted = prepare.format_entry(entry, with_audio=True, audio_root="/data")

    assert formatted["audio_path"] == ["/data/mmau-pro/audio/example.wav", "/data/mmau-pro/already/absolute.wav"]
    assert formatted["messages"][0]["audios"][0]["path"] == "/data/mmau-pro/audio/example.wav"
